...
import ch.cpnv.angrywirds.Models.Stage.Poo;
...
public class Play extends GameActivity implements InputProcessor {
    ...
    public Play(int lvlid) {
        super();

        babble = new ArrayList<Bubble>();
        vocabulary = VocProvider.vocabularies.get(lvlid-1);
    ...
    public void update(float dt) {
        ...
        if (tweety.collidesWith(waspy)) {
            ...
            AngryWirds.gameActivityManager.push(new GameOver(new Title("Game Over\n score: "+ ScoreBoard.score)));
        }
        if (tweety.getSprite().getX() > WORLD_WIDTH - Bird.WIDTH) {
            tweety.reset();
            poo.reset();
        }
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        ...
        if (tweety.getState() == Bird.BirdState.fly && !poo.isPooped) {
            poo.setPosition(tweety.getPosition());
            poo.isPooped = true;
        }
        return false;
    }
